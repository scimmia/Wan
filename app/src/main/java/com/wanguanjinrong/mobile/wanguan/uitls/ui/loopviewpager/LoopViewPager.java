...
package com.wanguanjinrong.mobile.wanguan.uitls.ui.loopviewpager;
...
public class LoopViewPager extends ViewPager {
    ...
    static class AdSwitchTask implements Runnable {
        ...
        AdSwitchTask(LoopViewPager convenientBanner) {
            this.reference = new WeakReference<LoopViewPager>(convenientBanner);
        }

        @Override
        public void run() {
            LoopViewPager convenientBanner = reference.get();

            if(convenientBanner != null){
                int count = convenientBanner.getAdapter().getCount();
                int index = convenientBanner.getCurrentItem();
                index = (index + 1) % (count); //这里修改过
                convenientBanner.setCurrentItem(index, true);

                convenientBanner.postDelayed(convenientBanner.adSwitchTask, 3000);
            }
        }
    }
}
